...
class VisionTransformerAvgPoolFeat(VisionTransformer):
    """Vision Transformer that returns the [CLS] token and the patch tokens from the last block."""
    def forward(self, x, n=1, return_patch_avgpool=True):
        return super().forward_return_n_last_blocks(x, n, return_patch_avgpool)


class VisionTransformerFeat(nn.Module):
    """ Vision Transformer """
...
class DistilledVisionTransformerCls(VisionTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dist_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        num_patches = self.patch_embed.num_patches
...
class DistilledVisionTransformerShape(VisionTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dist_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        num_patches = self.patch_embed.num_patches
...
@register_model
def dino_small_feat(patch_size=16, pretrained=False, **kwargs):
    model = VisionTransformerFeat(
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model_url = {
        16: "https://dl.fbaipublicfiles.com/dino/dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth",
...
@register_model
def dino_small_avgpoolfeat(patch_size=16, pretrained=False, **kwargs):
    model = VisionTransformerAvgPoolFeat(
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model_url = {
        16: "https://dl.fbaipublicfiles.com/dino/dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth",
...
@register_model
def dino_small_dist_cls_feat(patch_size=16, pretrained=False, **kwargs):
    model = DistilledVisionTransformerCls(
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def dino_small_dist_shape_feat(patch_size=16, pretrained=False, **kwargs):
    model = DistilledVisionTransformerShape(
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
